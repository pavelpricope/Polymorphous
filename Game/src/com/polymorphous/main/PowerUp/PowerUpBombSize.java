...
public class PowerUpBombSize extends GameObject {

    int width = Game.BLOCK_SIZE;
    int height = Game.BLOCK_SIZE;
    private Polygon bounds;

    public PowerUpBombSize(int x, int y, ID id) {
        super(x, y, id);
        //up
        double sx = x + width / 2;
    ...
    public void render(GraphicsContext g) {
        g.drawImage(Game.getImg.BOMB_SIZE_P_UP(0), x, y, width, height);
    }
    ...
    private void collide() {
        ...
        for (Player player : Game.getPlayersHandler().players) {
            if (Collision.checkPolygonsCollision(getBounds(), player.getBounds())) {
                ...
                Game.audioManager.sfx.bombSize.play();
